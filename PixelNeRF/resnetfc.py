...
class ResnetFC(nn.Module):
    ...
    def forward(
        self, zx, ns=-1, combine_inner_dims=(1,), combine_index=None, dim_size=None
    ):
        """
        ...
        with profiler.record_function("resnetfc_infer"):
            ...
            for blkid in range(self.n_blocks):
                # print(f"blkid={blkid}, ns={ns}, x.shape={x.shape}")
                if blkid == self.combine_layer and ns > 1:
                    # x = combine_interleaved(x, combine_inner_dims, self.combine_type)
                    # print(f"combining: ns={ns}, x.shape={x.shape}")
