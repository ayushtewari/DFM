...
class JsonIndexDataset(DatasetBase, ReplaceableBase):
    ...
    Args:
        ...
        center_crop: Enable cropping of the image around the center of the image instead of around foreground mask 
    ...
    def _load_crop_fg_probability(
        ...
        if (self.load_masks or self.box_crop) and entry.mask is not None:
            ...
            if self.box_crop:
                if self.center_crop:
                    # foreground_center = torch.tensor([bbox_xywh[0] + bbox_xywh[2] / 2.0, bbox_xywh[1] + bbox_xywh[3] / 2.0]).round().long()
                    # foreground_center = torch.tensor([width // 2, height // 2])
                    # import pdb; pdb.set_trace()
                    height, width = entry.image.size
                    foreground_center = torch.tensor([width // 2, height // 2])

                    square_size = min(height // 2, width // 2)

                    clamp_bbox_xyxy = torch.tensor(
                        [
                            foreground_center[0] - square_size,
                            foreground_center[1] - square_size,
                            foreground_center[0] + square_size,
                            foreground_center[1] + square_size,
                        ]
                    )
                    # print(f"original size {height}x{width}, cropped size {clamp_bbox_xyxy} squre_size {square_size}")
                else:
    ...
    def _get_pytorch3d_camera(
        ...
        # print(f"principal point {principal_point} focal length {focal_length} image_size {entry.image.size}")
        # import pdb; pdb.set_trace()
    ...
    def _resize_image(
        ...
        minscale = min(image_height / image.shape[-2], image_width / image.shape[-1],)
    ...
    def get_frame_numbers_and_timestamps(
        self, idxs: Sequence[int], subset_filter=None
...
def _get_clamp_bbox(
    bbox: torch.Tensor, box_crop_context: float = 0.0, image_path: str = "",
) -> torch.Tensor:
